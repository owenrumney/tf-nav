...
export class TerraformWatcher implements vscode.Disposable {
  ...
  private onFilesUpdatedEmitter = new vscode.EventEmitter<{
    files: string[];
    index: ProjectIndex;
  }>();
  private onFilesAddedEmitter = new vscode.EventEmitter<{
    files: string[];
    index: ProjectIndex;
  }>();
  private onFilesDeletedEmitter = new vscode.EventEmitter<{
    files: string[];
    index: ProjectIndex;
  }>();
  private onParseErrorsEmitter = new vscode.EventEmitter<
  ...
  private async buildInitialIndex(): Promise<void> {
    try {
      ...
      const result = await this.workerManager.buildIndex(
        terraformFiles,
        {
        ...
        (progress: WorkerProgressUpdate) => {
          this.log(
            `Progress: ${progress.processed}/${progress.total} - ${progress.currentFile}`
          );
        }
      );
      ...
      this.log(
        `Initial index built: ${result.stats.totalBlocks} blocks from ${result.stats.filesProcessed} files`
      );
  ...
  private setupFileWatcher(): void {
    ...
    this.fileWatcher =
      vscode.workspace.createFileSystemWatcher('**/*.{tf,tf.json}');
  ...
  private async processUpdates(): Promise<void> {
    ...
    try {
      ...
      this.log(
        `Update complete. Index now has ${this.currentIndex.blocks.length} blocks`
      );
    } catch (error) {
  ...
  private async processChangedFiles(
    ...
    const changedFiles = filesToReparse.filter(

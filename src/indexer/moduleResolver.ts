...
export function resolveModuleSource(
  ...
  if (source.includes('/') && !source.includes('://')) {
    ...
    return {
      ...
      error:
        'Registry modules cannot be resolved locally without terraform init',
    };
  }
 ...
 */
function resolveTerraformModuleCache(
  source: string,
  baseDir: string
  ...
  while (currentDir !== path.dirname(currentDir)) {
    ...
    if (
      fs.existsSync(potentialTerraformDir) &&
      fs.statSync(potentialTerraformDir).isDirectory()
    ) {
  ...
  try {
      ...
      if (module.Source === source || module.Key === source) {
        ...
        if (
          ...
          return {
            resolved: true,
            modulePath,
            resolutionType: module.Source.includes('://')
              ? 'git'
          ...
          console.warn(
            `[ModuleResolver] Module path does not exist or is not directory`
          );
        }
      }
    }
    ...
    console.warn(
      `[ModuleResolver] Failed to read Terraform modules manifest: ${error}`
    );
    return null;
  }
}

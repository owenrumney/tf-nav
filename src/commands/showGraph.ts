...
export async function showGraph(
  ...
  if (!index) {
    vscode.window.showWarningMessage(
      'No Terraform index available. Please wait for indexing to complete.'
    );
    return;
  }
  ...
  if (!index.refs || index.refs.length === 0) {
    vscode.window.showInformationMessage(
      'No dependencies found in the current workspace.'
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `[ShowGraph] Opened dependency graph${focusMsg} - ${index.refs.length} dependencies found`
    );
  } catch (error) {
...
export function registerShowGraphCommand(
  ...
  return vscode.commands.registerCommand(
    'tfnav.showGraph',
    async (treeItem?: TerraformTreeItem) => {

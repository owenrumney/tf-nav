 ...
 */
async function byteOffsetToPosition(filePath: string, byteOffset: number): Promise<vscode.Position> {
  try {
...
export async function revealInEditor(item: TerraformTreeItem): Promise<void> {
  if (!item.address || !item.resourceUri) {
    vscode.window.showErrorMessage('Cannot reveal: No location information available');
    return;
  }
  ...
  try {
    ...
    const startPosition = await byteOffsetToPosition(filePath, item.address.range.start);
    const endPosition = await byteOffsetToPosition(filePath, item.address.range.end);
    ...
    const safeStartChar = Math.min(startPosition.character, document.lineAt(safeStartLine).text.length);
    const safeEndChar = Math.min(endPosition.character, document.lineAt(safeEndLine).text.length);
    ...
    editor.selection = new vscode.Selection(safeStartPosition, safeStartPosition);
 ...
 */
export function registerRevealCommand(context: vscode.ExtensionContext): vscode.Disposable {

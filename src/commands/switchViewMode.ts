...
export async function switchViewMode(): Promise<void> {
  try {
    ...
    const modeDescription = newMode === 'type' ? 'by type (grouped by resource kinds)' : 'by file (grouped by files)';
 ...
 */
export function registerSwitchViewModeCommand(context: vscode.ExtensionContext): vscode.Disposable {
  return vscode.commands.registerCommand('tfnav.switchViewMode', switchViewMode);
}

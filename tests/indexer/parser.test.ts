...
variable "instance_count" {
  ...
  validation {
    ...
    error_message = "Instance count must be positive."
      ...
      const instanceCount = result.blocks.find(
        (b) => b.name === 'instance_count'
      );
...
resource "aws_instance" "web" {
  ami = "ami-12345"
      ...
      expect(result.blocks).toHaveLength(1);
      expect(result.blocks[0].modulePath).toEqual([
        'module.vpc',
        'module.compute',
      ]);
    });
...
resource "azurerm_virtual_machine" "vm" {
  name = "test-vm"
      ...
      const googleResource = result.blocks.find(
        (b) => b.kind === 'google_compute_instance'
      );
      expect(googleResource?.provider).toBe('google');

      const azureResource = result.blocks.find(
        (b) => b.kind === 'azurerm_virtual_machine'
      );

...
describe('TerraformFileCollector', () => {
  ...
  beforeAll(() => {
    ...
    if (!validation.isValid) {
      throw new Error(`Test workspace validation failed:\n${validation.errors.join('\n')}`);
    }
  });
  ...
  describe('findTfFiles', () => {
          ...
          const matchingFiles = allFiles
            ...
            .filter(file => {
              ...
              if (patternStr.includes('*.tf')) {
                return file.path.endsWith('.tf') && !file.path.endsWith('.tf.json');
              }
              return false;
            })
            .filter(file => {
    ...
    it('should discover both .tf and .tf.json files', async () => {
      ...
      const tfFiles = discoveredFiles.filter(file => 
      ...
      const s3JsonFile = tfJsonFiles.find(file => file.includes('s3.tf.json'));
    ...
    it('should discover files in nested module directories', async () => {
      ...
      const cloudwatchFiles = moduleFiles.filter(file => file.includes('/cloudwatch/'));
  ...
  describe('disposal', () => {
    it('should dispose output channel when disposed', () => {
      const mockOutputChannel = (global as any).mockVSCode.window.createOutputChannel();

...
describe('File Watcher Integration', () => {
  ...
  it('should build initial index when started', async () => {
    ...
    if (currentIndex) {
      ...
      console.log(`✅ File watcher integration test: Built index with ${currentIndex.blocks.length} blocks`);
  ...
  it('should handle rebuild index operation', async () => {
    ...
    console.log(`✅ Index rebuild test: Maintained ${rebuiltIndex?.blocks.length} blocks`);
  });
  ...
  it('should demonstrate complete workflow', async () => {
    ...
    try {
      ...
      if (currentIndex) {
        ...
        console.log(`   - Block types: ${Array.from(currentIndex.byType.keys()).join(', ')}`);
  ...
  it('should handle file watcher setup correctly', async () => {
    ...
    const createWatcherSpy = jest.spyOn(global.mockVSCode.workspace, 'createFileSystemWatcher');
  ...
  it('should demonstrate debouncing capability', () => {
    ...
    console.log('✅ Debouncing configured correctly (50ms for testing, 250ms for production)');
  });
  ...
  it('should show incremental update capability', async () => {
    ...
    console.log(`   - Main blocks array has ${initialIndex?.blocks.length} blocks`);
  });
});

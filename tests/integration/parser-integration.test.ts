...
describe('Parser Integration Tests', () => {
  ...
  it('should parse main.tf from test workspace', async () => {
    ...
    const awsResources = result.blocks.filter(
      (b) => b.blockType === 'resource' && b.provider === 'aws'
    );
    ...
    const vpc = result.blocks.find(
      (b) => b.kind === 'aws_vpc' && b.name === 'main'
    );
    expect(vpc).toBeDefined();
    expect(vpc?.blockType).toBe('resource');
    expect(vpc?.provider).toBe('aws');

    const dbModule = result.blocks.find(
      (b) => b.blockType === 'module' && b.name === 'database'
    );
  ...
  it('should parse variables.tf from test workspace', async () => {
    ...
    const result = await TerraformParserFactory.parseFile(
      variablesTfPath,
      content
    );
  ...
  it('should parse data.tf from test workspace', async () => {
    ...
    const callerIdentity = result.blocks.find(
      (b) => b.kind === 'aws_caller_identity'
    );
    expect(callerIdentity).toBeDefined();
    expect(callerIdentity?.provider).toBe('aws');

    const ubuntuAmi = result.blocks.find(
      (b) => b.kind === 'aws_ami' && b.name === 'ubuntu'
    );
  ...
  it('should parse module files from test workspace', async () => {
    ...
    const dbSubnetGroup = result.blocks.find(
      (b) => b.kind === 'aws_db_subnet_group'
    );
